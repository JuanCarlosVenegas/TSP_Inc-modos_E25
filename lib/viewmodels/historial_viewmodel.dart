...
class HistorialViewModel {
  ...
  Map<String, List<PickupRequest>> groupByDate(QuerySnapshot snapshot) {
    ...
    for (var item in items) {
      ...
      groupedByDate.putIfAbsent(date, () => []).add(PickupRequest.fromJson(item));
    }

    return groupedByDate;
  }

  Future<void> updatePickupStatus(PickupRequest pickupRequest, String filterBy) async {
  String newStatus;
  ...
  try {
    ...
    DocumentSnapshot doc = await _firestore.collection('pickup_requests').doc(pickupRequest.requestId).get();
